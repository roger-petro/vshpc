 ...
 * @returns
 */
export const convertISODate2LocalTime = (dateStr:string) => {
    const date = new Date(dateStr+"+03:00"); //zulu time
    return date.toISOString().toLocaleString().split('T')[0] + ' ' +
        date.toISOString().toLocaleString().split('T')[1].split('.')[0];
};

export const convertISOZDate2LocalTime = (isoString) => {
 ...
 */
export const histInfoOrder : Array<keyof KibanaPaged.Source> = [
    'jobid',
    'job_name',
    'state', ,
    'elapsed',
...
export function getPromiseFromVScodeEvent(item, event, timeout = 10000) {
    return new Promise<object|null>((resolve) => {
        let timer = null;
        const listener = (event) => {
            let data = 'data' in event? event['data'] : event['detail'];
