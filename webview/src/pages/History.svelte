<script lang="ts">
    ...
    const get_kibana = async (
        ...
        console.log(
            `Total de entradas: ${kibana._hitsTotal}, total de páginas: ${Math.ceil(kibana._hitsTotal / itemsPerPage)}`,
        );
        ...
        histInfoOrder.forEach(key => {
            if (key) {
                const formattedVal = formatValue(
                    key,
                    (rows[elem]._source as Record<string, any>)[key] as string,
                );
    ...
    const linkAvailable = (elem: KibanaPaged.Source) => {
        return (
            elem.job_name.match(/\.(data|dat|xml|DATA|geo|gdt)$|((data|dat)\.[a-f0-9]{8})$/i) ||
            elem.work_dir.match(/.*\.cmpd.*/i)
        );
    };
    ...
    function changeSelection(jobid: number, isChecked: boolean) {
        if (isChecked) {
            selected.add(jobid);
        } else {
            selected.delete(jobid);
        }
        // para disparar reatividade (se precisar transformar em array, use Array.from(selected))
    ...
    <div class="wrapper">
        ...
        <div class="first-line">
            <div id="top" class="pageTop">
                ...
                <span class="hint" title="Filtrar quantos dias desejados de histórico">
                    <label class="text">
                        Dias de histórico:
                        <input bind:value={days} size="12" placeholder="days" />
                    </label>
                </span>
                ...
                <button
                    on:click={() =>
                        setPage(page + 5 > totalPages ? totalPages - 1 : page + 5, true)}
                    >&raquo;</button
                >
                <Sep w={1} />
        ...
        <div class="third-line">
            ...
            <div class="grid2">
                ...
                <div class="grid1-header">Abrir...</div>
                {#each rows as job, index (job._source.jobid)}
                    ...
                    <div class="grid1-column">
                        {convertISODate2LocalTime(job._source['@submit'], '+03:00')}
                    </div>
                    ...
                    {#if linkAvailable(job._source)}
                        ...
                        <div class="grid1-column">
                            <a on:click={() => openLog(job._source.jobid)}>{job._source.job_name}</a
                            >
                        </div>
                    {:else}
