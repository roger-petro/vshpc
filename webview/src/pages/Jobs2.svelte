<script lang="ts">
    ...
    function refresh(checked:boolean) {
        listJobs(selectedUser,selectedAccount,itemsPerPage)
        if (interval) clearInterval(interval)
        if (checked) {
    ...
    <div class="wrapper">
        ...
        <div class="first-line">
                <div id="top" class="pageTop">
                    ...
                    <input placeholder={getMeta("user")} class="v_i" type="text" max="4" name="user" bind:value={selectedUser} style="width:50px">
                    <Sep w={1}/>
                    ...
                    <Sep w={1}/>
                    <button class="button green"  on:click={()=>listJobs(selectedUser,selectedAccount,itemsPerPage)} >Recarregar</button>
                    <Sep w={2}/>

                    {#if rows.length > 0}
                        {#if selectedRows.findIndex(e=>e===true)===-1}
                            <button class="button_sized_1"  on:click={()=>{selectedRows.forEach((e,i)=>
                                {if (rows[i].user === getMeta("user"))
                                    {selectedRows[i]=true}})
                                }}>Selecionar Todos</button>
                        {:else}
                            <button class="button_sized_1" on:click={()=>{selectedRows.forEach((e,i)=>selectedRows[i]=false)}}>Desmarcar</button>

                        {/if}
                        <button disabled={selectedRows.findIndex(e=>e===true)===-1}
                             on:click={()=>{modal_show=true}}>Matar</button>


                        <button disabled="{page === 0}" on:click={() => setPage(page-5>0?page-5:0, true)}>&laquo;</button>
                        ...
                        {/each}
                        <button disabled="{page === pages.length-1}"  on:click={() => setPage(page+5>pages.length?pages.length-1:page+5, true)}>&raquo;</button>
                    {/if}
                    <Sep w={1}/>
        ...
        <div class="second-line">
            {@html statusMessage.split(':')[0]}<Sep w={2}/>{#if loading}Carregando...{localTickTack}<Sep w={2}/>{/if}
        </div>
        <div class="third-line">
            ...
            <div class="grid1">
                ...
                {#each currentPageRows as  job, index (job.id)}
                    ...
                    /></div>
                    <div class="grid1-column"><a href="/" on:click={()=>{moreInfo(job.id)}}>{job.id}</a></div>
                    <div class="grid1-column"><a href="/" on:click={()=>{openPortal(job.user)}}>{job.user}</a></div>
...
.progress-cell {
    position: relative;
    background-color: #f1f1f1; /* Cor de fundo opcional para a célula */
    width: 100%;
...
.progress-bar {
    ...
    left: 0;
    height: 100%;
    background-color: rgba(255, 255, 0, 0.5); /* Amarelo com 50% de transparência */
