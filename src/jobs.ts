...
async function sendJob(model: string, option: SubmitOption, specificHash: string | null) {
    ...
    if (option === SubmitOption.git) {
        ...
        if (await repository.getLocalMetaData(specificHash)) {
            ...
            if (!(await repository.getRemoteMetaData())) {
                PubSub.publish(
                    LogOpt.vshpc,
                    `> submit: Pasta remota possivelmente não existia ou não era controlada por git`,
                );
            } else {
    ...
    if (ret && ret.success) {
        ...
        PubSub.publish(
            LogOpt.bar,
            `Job criado com: ${ret.message}. Faltam ${jobQueueArray.length} jobs para serem submetidos`,
        );
...
export function showJob2Kill(jobAr: JobArrayType): boolean {
    ...
    if (jobAr && Object.keys(jobAr).length === 0) {
        return false;
    }
    vscode.window
        ...
        if (ret && ret.code === 0 && ret.multiline !== undefined && ret.multiline.length > 0) {
            ...
            for (let i = 1; i < ret.multiline.length; i++) {
                ...
                if (job.length !== outputFormat.split(';').length) {
                    PubSub.publish(
                        LogOpt.vshpc,
                        '> getJobs: Quantidade de informações insuficientes',
                    );
                    return [];
                }
                jobsArray.push({
            ...
            PubSub.publish(
                LogOpt.vshpc,
                `> getJobs: Algo errado ao retornar os jobs. Código: ${ret.code} stdout: ${ret.stdout} stderr: ${ret.stderr}`,
            );
            return [];
        }
    } catch (e) {
 ...
 * @returns
 */
export async function getUserJobUpdates(
    settings: SettingsType,
    payload: any,
): Promise<JobArrayType[]> {
    let filterUser = '';
    ...
    try {
        ...
        if (ret && ret.code === 0 && ret.multiline !== undefined && ret.multiline.length > 0) {
            ...
            PubSub.publish(
                LogOpt.vshpc,
                `> getUpdateJobs: Algo errado ao retornar os jobs. Código: ${ret.code} stdout: ${ret.stdout} stderr: ${ret.stderr}`,
            );
            return [];
        }
    } catch (e) {
...
export function formatJobs(jobs: JobArrayType[] | null): string {
    if (jobs === null || jobs.length === 0) {
        return `Nenhum job encontrado ( ${new Date()})`;
    }
    let formattedJob = [];
    let uri: vscode.Uri;
...
export async function killJob(settings: SettingsType, id: string): Promise<RetMsg> {
    ...
    if (ret) {
        ...
        if (ret.code === 0) {
            ...
            return {
                success: false,
                message: ret.multiline ? ret.multiline.join(' ') : ret.stderr,
            };
        }
    }
