...
import { SettingsType, RetMsg, LogOpt, SubmitOption, RepoType, GitReferencePointers} from './types';
                ...
                git.listRemote(['--heads','--tags'], (err, data) => {
                    if (!err) {
                        ...
                        const lines = data.trim().split('\n').map((value) => {
                            ar.push({   'sha1': value.trim().split(/\t|\s+/)[0],
    ...
    return new Promise( resolve => {
        ( async (git) => {
            ...
            try {
                ...
                if (! server ) {
                    PubSub.publish(LogOpt.vshpc,"> getGitServer: Não havia servidor remoto no repositório");
                    resolve("");
                }
 ...
 */
export function getGitProjectName(uRI: string): string|null {
    let ret = path.basename(uRI,'.git');
    if (!ret || ret.length === 0) {
            ...
            try {
                await simpleGit(localFolder).branchLocal((err, result) => {
                    if (!err && result) {
                        ...
                        PubSub.publish(LogOpt.vshpc,`> getBranchName: Path solicitado ${localFolder}`);
                        resolve(null);
                    }
                });
            } catch(e) {
 ...
 */
export async function getBranchesTips(localFolder: string, remote: boolean, hash: string): Promise<string[]> {
    const git: SimpleGit = simpleGit(localFolder);
    return new Promise ( (resolve,reject) => {
        ( async (git,localFolder) => {
            try {
 ...
 * observando o folder local (aberto no Workspace)
 */
export async function getCurrentTags(localFolder: string, specificHash: string | null): Promise<string[]> {
    const git: SimpleGit = simpleGit(localFolder);
    return new Promise ( (resolve,reject) => {
        ( async (git,localFolder) => {
            try {
 ...
 */
 export async function getCommitLog(localFolder: string, sha1: string): Promise<string[]> {
    ...
    return new Promise ( (resolve) => {
        ( async (git) => {
                git.raw(['log', '--pretty=format:%aN;%ae;%cd;%s',
                        '--date=format-local:%Y.%m.%d;%H:%M:%S','-n 1', sha1], (error,result) => {
                    if (error) {
                        console.log(error);
                        resolve(['']);
                    };
...
export async function checkRemoteGitVersion(settings: SettingsType): Promise<string>{
    const ret = await sendSSHcommand(`${evalGitPath()} --version`, [''],
        settings.cluster,settings.user,settings.passwd, settings.privRsaKey );
    if (!ret) {
        PubSub.publish(LogOpt.vshpc, "> checkRemoteGitVer: Erro ao verificar a versão do git remoto");
 ...
 * @returns boolean
 */
export async function checkRemoteFolder(settings: SettingsType,remoteFolder: string): Promise<boolean>{
    const ret = await sendSSHcommand(`[ -d "${remoteFolder}" ]; echo $?`, [''],
        settings.cluster,settings.user,settings.passwd, settings.privRsaKey );
    if (!ret) {
    ...
    if (ret) {
        PubSub.publish(LogOpt.vshpc,`> checkRemoteFolder: ${remoteFolder} `+
 ...
 */
export async function gitActions(settings: SettingsType, option: SubmitOption, repo: Repository): Promise<RetMsg> {

    if (repo.getRemotePath() === repo.getRemoteClonePath()) {
        PubSub.publish(LogOpt.vshpc, '> gitActions: Por proteção não é possível fazer pull sobre o diretório de trabalho');
    ...
    try {
        ...
        if ( option === SubmitOption.git) {
            PubSub.publish(LogOpt.vshpc,`> gitActions: Tentando um Clone na pasta ${repo.getRemoteClonePath()}`);
            ...
            cmd = `[ ! -d ${remotePath} ] && mkdir -p ${remotePath} && ` +
            ` cd ${remotePath} && ` +
        ...
        if (ret && ret.code === 0) {
            ...
            if ( created === true) {
                if (option === SubmitOption.git) {PubSub.publish(LogOpt.progress,`Realizadas eventuais operações de git 🙂`);}
                PubSub.publish(LogOpt.vshpc,`> gitActions: Resultado das operações git abaixo:`);
                PubSub.publish(LogOpt.vshpc,`> gitActions: código: ${ret.code} mensagem: ${ret.stdout}`);
                PubSub.publish(LogOpt.vshpc, `> gitActions: Mensagens extras:\n${ret.stderr}`);
                return { success: true, message: ret.stdout};
            } else {
                PubSub.publish(LogOpt.vshpc,`> gitActions: O git falhou em criar a pasta com clone.`);
                PubSub.publish(LogOpt.vshpc, `> gitActions: Mensagens extras:\n${ret.stderr}`);
                PubSub.publish(LogOpt.progress,`O git falhou em criar a pasta com clone. Veja os logs`);
        ...
        PubSub.publish(LogOpt.vshpc,`> gitActions: Erro! SSH não me retornou nada. Talvez problema de conexão`);
