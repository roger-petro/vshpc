...
const checks = {
    ...
    isRootFolder:
        '- Pasta aberta na raiz do projeto: (avaliado apenas se tiver controlado por git)',
...
async function checkAsyncs() {
    ...
    checks.sshKey = `- SSH por chave: ${
        settings.usePassword
            ? 'Configuração forçou o uso de senha. Veja em settings.'
            : metaData[1].code === 0
            ? 'OK!'
            : 'Falhou'
    }`;
    ...
    if (repo.getIsLocalRepo() === true) {
        ...
        checks.isRootFolder = `- Pasta aberta na raiz do projeto: ${
            repo.getIsLocalRootRepo() ? 'Sim' : 'Não'
        }`;
        ...
        checks.tag = `- Tags do checkout atual: ${
            repo.getTag() === '0' ? 'Nenhuma' : repo.getTag()
        }`;
        ...
        checks.remotePathExists = `- Existe o caminho do projeto visto pelo Linux: ${
            repo.getIsRemotePath() ? 'Sim' : 'Não'
        }`;
        checks.remotePath = `- Caminho do projeto visto pelo Linux: ${
            repo.getRemotePath()
                ? repo.getRemotePath()
                : 'Erro! Verifique se o "de-para" contempla a pasta do projeto aberta'
        }`;
        ...
        checks.branchTips =
            '- Branches e tags no topo deste commit: ' +
            (await getBranchesTips(settings.workdir, false, repo.getHash('full'))).join(' ');
        checks.gitDestinationEndExists =
            '- Pasta final de destino do git existe: ' +
            (repo.getIsRemoteClonePath() ? 'OK!' : 'Não existe');
    } else {
        //projeto não tem git
    ...
    if (metaData[0].code === 0 || metaData[1].code === 0) {
        ...
        if (testRemote.stdout === '0') {
            ...
            checks.checkRemoteAccess =
                '- Acesso ao GitLab pelo cluster: ERRO! ' +
