...
export async function getModelName(clickedFile : vscode.Uri | null) {
	...
	if (clickedFile) {
		...
		const simulator = VSHPC.simulators.find((item) => item.solvers.find(sol => sol === settings.solverName)) as Simulator;
		if (simulator && settings.workdir!=='') { //vscode.workspace.workspaceFolders !== undefined) {
			...
			if (path.extname(modfile) === '.' + simulator.ext || path.extname(modfile) === '.' + simulator.ext.toUpperCase()) {
				//console.log('Modelo que foi selecionado ao clicar :' + modfile + " para o solver:" + settings.solverName);
				model = modfile;
			} else {
				PubSub.publish(LogOpt.vshpc, `> getModelName: o arquivo ${modfile} não foi identificado como um  modelo pela extensão`);
			}
		} else {
...
export function getOpenedModelName() {
	...
	const simulator = simulDefs.find((item: Simulator) => item.solvers.find(sol => sol === settings.solverName));
	if (simulator) {
		let doc = vscode.window.activeTextEditor;
		if (simulator && doc && vscode.workspace.workspaceFolders !== undefined && vscode.window.activeTextEditor) {
			...
			if (folder) {
				...
				if (path.extname(model) === '.' + simulator.ext || path.extname(model) === '.' + simulator.ext.toUpperCase()) {
...
export async function  pickSiblingModels(uri:vscode.Uri) : Promise<string[]> {
	...
	if (!checkAccountSettings() || !checkSubmitSettings()) {
		PubSub.publish(LogOpt.vshpc, `> pickSiblingModels: configurações carregadas: ${JSON.stringify(settings)}`);
		return [];
	}
	let simulator = simulDefs.find((item : Simulator) => item.solvers.find(sol => sol === settings.solverName)) as Simulator;
	if (!simulator) {
		return [];
	}
	...
	if (rootFolder) {
		...
		const relative = new vscode.RelativePattern(rootFolder.uri.fsPath, `${subdir}*.{${simulator?.ext},${simulator.ext.toUpperCase()}}`);
		...
		for (let _path of ar) {
			if (_path[0] === '/') {	_path = _path.substring(1);} //retira o primeiro "/"
		}
		return ar;
	}
	return [];
...
export async function  pickModel() : Promise<WorkspaceModelFolder|undefined> {
	...
	if (vscode.workspace.workspaceFolders) {
		for (let datFile of datFiles) {
			...
			else  { ar.push(`[${workspaceNumber}]: ${model}`);}
			modelFolderArr.push({model: model, path : folderPath, workspaceindex: workspaceNumber });
		}
	} else {
		return;
	}
	...
	const selection = await vscode.window.showQuickPick(ar,{"title": "Modelos (selecione para simular):"});
...
export function checkOptions(params: {models:string[], hash?: string}) {
	...
	for (const model of params.models) {
		...
		if(uri.search(/\s+/)>-1) {
			PubSub.publish(LogOpt.toast_error,`Nomes de modelos ou caminhos com espaço não funcionam no cluster`);
			return false;
		}
