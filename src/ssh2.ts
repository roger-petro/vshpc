...
export async function sendSSHcommand(command: string, params: string[], cluster: string,
    user: string, passwd: string, privRsaKey: string): Promise<SSHMsg> {
    ...
    return new Promise ((resolve) => {
        ...
        if (privRsaKey !== "" && key) {
            ...
            if (sshUserConfig.password === null) {
                PubSub.publish(LogOpt.vshpc,"> ssh2: Nao é possível conectar sem senha ou erro ao decifrar");
                resolve({
                    code : 128,
                    stderr: "",
